...
# Private channel: "-1001234567890" (get from @userinfobot or @getidsbot)
DEFAULT_CHANNEL="@arbitrage_pro_signals"
...
# Private channels need numeric ID: -1001234567890
# Public channels use: @channelname
if channel.startswith('-100'):
    chat_id = channel  # Private channel ID
elif channel.startswith('@'):
    chat_id = channel  # Public channel
else:
    chat_id = '@' + channel  # Assume public channel name
...
try:
    ...
    try:
        ...
        err_msg = error_json.get('description', 'Bad Request')
        if 'chat not found' in err_msg.lower():
            print(f"❌ Channel not found: {chat_id}\n   Make sure bot is added to channel")
        elif 'bot is not a member' in err_msg.lower():
            print(f"❌ Bot not in channel: {chat_id}\n   Add bot as admin first")
        else:
            print(f"❌ Telegram: {err_msg}")
    except:
        print(f"❌ HTTP {e.code}: Check channel format")
except Exception as e:
...
case "$MSG_LOWER" in
  ...
  "📢 post to channel"*)
    ...
    MSG_CONTENT=$(echo "$MESSAGE" | sed 's/^📢 Post to Channel //i')
    
    if [ -n "$MSG_CONTENT" ]; then
        # User provided message - post to default channel
        RESULT=$(post_to_channel "$DEFAULT_CHANNEL" "$MSG_CONTENT")
        send_msg "$RESULT" ""
    else
        # No message yet - prompt user
        send_msg "📢 CHANNEL BROADCAST\n\nDefault: <code>$DEFAULT_CHANNEL</code>\n\nType your message:\n📢 Post to Channel Your message here\n\nOr:\n• post Your message\n• post @channel Your message\n\n⚠️ Private channels need numeric ID:\n1. Forward any message from channel to @getidsbot\n2. Copy the 'Forwarded from chat:' ID\n3. Use: post -1001234567890 Message" ""
    fi
  ...
  "post"*)
    ...
    # Check if first word starts with @ or is a channel ID (starts with -100)
    FIRST_WORD=$(echo "$REST" | awk '{print $1}')
    if [[ "$FIRST_WORD" == @* ]] || [[ "$FIRST_WORD" == -100* ]]; then
    ...
    if [ -n "$MSG_CONTENT" ]; then
        ...
        send_msg "📢 Usage:\n• post Your message (to default)\n• post @channel Message (to specific)\n• post -1001234567890 Message (private channel)" ""
    fi
    echo "✅ Posted!"
    ;;
